...
```{r}
GPS_track$diff_mins <- round(GPS_sf$diff_mins ,2)
GPS_track <- GPS_track %>%
...
```{r}
boxplot1=ggplot(GPS_track,aes(x="",y=GPS_sf$diff_mins))+geom_boxplot()+labs(title="Distribution of time gap")+theme_classic()
...
```{r}
GPS_track$point <- 0
GPS_track$point[GPS_sf$diff_mins >5 ] <- 1
GPS_track$point[GPS_sf$diff_mins <=5 ] <- 0
...
```{r}
Tracker_GPS <- 
aggregate(GPS_track$point, by=list(id=GPS_track$id,day=GPS_track$day), FUN=sum)
```
...
loyalty_people <- 
aggregate(loyalty_data$count_event, by=list(loyaltynum=loyalty_data$loyaltynum,day=loyalty_data$Day), FUN=sum)
...
cd_people <- 
aggregate(credit_debit$count_event, by=list(cdnum=credit_debit$last4ccnum,day=credit_debit$Day), FUN=sum)
