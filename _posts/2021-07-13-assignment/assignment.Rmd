...
aggregate_dataset <- loyalty_data %>% 
    ...
    summarize(Frequency = sum(count_event),Money_loyalty=sum(price))
head(aggregate_dataset)

credit_debit$timestamp <- as.Date(credit_debit$timestamp, "%m/%d/%Y")
aggregate_cc <- credit_debit %>% 
    group_by(timestamp,location) %>% 
    summarize(Frequency = sum(count_event),Money_cd=sum(price))
head(aggregate_cc)
...
```{r}
loyalty_money <- aggregate_dataset %>% group_by(Day,location) %>% summarise(max_loyal=max(Money_loyalty),freq_loyal=sum(Frequency))
head(loyalty_money)
count(loyalty_money)
```

```{r}
cc_money <- aggregate_cc %>% group_by(Day,location) %>% summarise(max_cc = max(Money_cd),freq_cc=sum(Frequency))
head(cc_money)
count(cc_money)
...
**Delete Duplicate columns(谨慎使用）**
```{r}
aggregate_cc= subset(aggregate_cc, select = -c(Day))
aggregate_dataset= subset(aggregate_dataset, select = -c(Day))
